...
def alpha_beta(board, player, alpha, beta, ply):
	...
	if player== "O":
		allow_m=board.available_moves()
		for action in allow_m:
			...
			score,_,exp_t=alpha_beta(board,temp_player,alpha,beta,ply-1)
			...
			if max_score<=alpha:
					return max_score,best_move,exp_t+exp
			if beta > max_score:
				beta = max_score
		...
		for action in allow_m:
			...
			if player=="X":
					...
					temp_player="X"
			score,move,exp_t=alpha_beta(board,temp_player,alpha,beta,ply-1)
			...
			exp+=exp_t
	return max_score,best_move,exp
...
if __name__ == '__main__':
	...
	player = b.create_board('336604464463')
	bestScore, bestMove, expansions = alpha_beta(b, player, -inf, inf, 5)
	print("The states that alpha_beta expanded:",count - temp)
	print("*************************************")
